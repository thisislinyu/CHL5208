...
# ) %>%
#   data.frame()
## file too large, cannot be pushed to Github, so I
## moved it to the CHL5208/data folder instead

lung_gene <- read.table("~/DLSPH/CHL5208/data/lung_gene.txt",
                        header = TRUE, sep = "\t"
) %>% data.frame()
...
phe_gen_dat <- inner_join(lung_phe1, lung_gene, by = "sample")
...
work_dat1 <- inner_join(clinical_dat1, phe_gen_dat,
...
uni_data <- work_dat1 %>%
  select(OS,OS.time,OS.time_yr,
         OS.time_month,
         project_id,
         age_at_diagnosis,
         gender,race,
         ajcc_pathologic_stage,cancer_stage)

gene_names <- colnames(lung_gene)[-1]

### Workdat2 for cox regression

work_dat2 <- work_dat1 %>%
  filter(!is.na(age_at_diagnosis)&!is.na(cancer_stage))

gene_dat <- work_dat2 %>%
  select(-colnames(.)[!(colnames(.)%in% gene_names)]) %>%
  apply(.,2,as.numeric)
...
outcome_dat <- work_dat2 %>% select(OS.time_month,OS) %>%
...
save(work_dat1,file="data/work_dat1.rdata") ## univariate date

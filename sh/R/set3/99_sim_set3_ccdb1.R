...
num_holder <- seq(1,nrow(hyperparameter_grid_tmp),1)
i <- 1

hyperparameter_grid <-hyperparameter_grid_tmp[i,]

library(parallel)

sim_set3_hyper_out <- NULL
for(i in c(1:length(sim_set3))){
  print(paste("i=",i))
  boot_lst <- sim_set3[[i]]
  ...
  sim_set3_hyper_out <- rbind(sim_set3_hyper_out,internal_res_tmp)
}


## in case overwrite
randomid <- i
save(sim_set3_hyper_out,file = paste0('data/sim_set3_hyper_out_',Sys.Date(),randomid,".rdata"))

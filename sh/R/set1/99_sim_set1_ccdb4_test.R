...
# Now, you can work with the loaded data in the global environment
# For example, print the object names in the global environment
...
hyperparameter_grid <-hyperparameter_grid_tmp[c( (num_holder[i]-3):num_holder[i]),]

##70*11 = 770 (70 hyperparameter_grid; 2 methods, 11 nfeatures)
#hyperparameter_grid <- expand.grid(qtl = 0.1,
#                                   Rf = 1.5)
## suppose 1 combination for 2 methods needs 10 min, then I need 15400 min which is

library(parallel)

sim_set1_hyper_out <- NULL

for(i in c(1)){
  #for(i in c(1:length(sim_set1))){
  print(paste("i=",i))
  boot_lst <- sim_set1[[i]]
  ...
  sim_set1_hyper_out <- rbind(sim_set1_hyper_out,internal_res_tmp)
}
## in case overwrite
randomid <- (runif(1,1,2)*10000000) %>% round(0)
save(sim_set1_hyper_out,file = paste0('data/sim_set1_hyper_out_',Sys.Date(),randomid,".rdata"))
